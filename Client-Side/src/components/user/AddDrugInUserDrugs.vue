...
const handleExpImageChange = () => {
  errors.value.exp_img = ''; 
};
...
const submitForm = async (event) => {
  ...
  if (!exp_date.value) {
    errors.value.exp_date = "Expire Date is required";
  }
  if (!price.value) {
  ...
  // Validate Expire Image
  const expireImageInput = document.getElementById("ExpireImage");
  if (!expireImageInput.files || expireImageInput.files.length === 0) {
    errors.value.exp_img = "Expire Image is required";
  }
